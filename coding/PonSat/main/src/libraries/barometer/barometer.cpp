...
void Barometer::measure()
{
    ...
    pressure = (uint16_t)(p + (var1 + var2 + ((double)dig_P7)) / 16.0) / 2;
}
...
uint64_t Barometer::getTemperature()
{
    return temperature;
}
uint16_t Barometer::getHeight()
{
    return uint16_t(44330 * (1.0 - pow(2 * pressure / 101325.0, 0.1903)) - h0);
}